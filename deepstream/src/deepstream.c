...
parse_pose_from_meta(NvDsBatchMeta *batch_meta, NvDsFrameMeta *frame_meta, NvDsObjectMeta *obj_meta)
{
  ...
  if (!obj_meta->mask_params.data || obj_meta->mask_params.size < sizeof(float) * 3 ||
      obj_meta->mask_params.width == 0 || obj_meta->mask_params.height == 0) {
    return;
  }
  ...
  for (guint i = 0; i < G_N_ELEMENTS(skeleton); ++i) {
    if ((guint) skeleton[i][0] > num_joints || (guint) skeleton[i][1] > num_joints) {
      continue;
    }
...
main(gint argc, char *argv[])
{
  ...
  if (STREAMMUX_BATCH_SIZE == 0 || STREAMMUX_WIDTH == 0 || STREAMMUX_HEIGHT == 0) {
    g_printerr("ERROR - Streammux batch size, width and height must be greater than zero\n");
    return -1;
  }

  gint current_device = -1;
  cudaError_t cuda_status = cudaSetDevice(GPU_ID);
  if (cuda_status != cudaSuccess || cudaGetDevice(&current_device) != cudaSuccess) {
    g_printerr("ERROR - Cannot select CUDA device %u: %s\n", GPU_ID, cudaGetErrorString(cuda_status));
    return -1;
  }
  struct cudaDeviceProp prop;
  if (cudaGetDeviceProperties(&prop, current_device) != cudaSuccess) {
    g_printerr("ERROR - Cannot query CUDA device %d\n", current_device);
    return -1;
  }
  ...
  /* Play file sources at 0.75x while keeping every frame for inference. */
  if (!gst_element_seek(pipeline, 0.75, GST_FORMAT_TIME,
          GST_SEEK_FLAG_FLUSH | GST_SEEK_FLAG_ACCURATE,
          GST_SEEK_TYPE_SET, 0, GST_SEEK_TYPE_NONE, GST_CLOCK_TIME_NONE)) {
    g_printerr("WARNING - Failed to set 0.75x playback rate\n");
  }
