...
struct Detector::Impl {
    ...
    auto initialize(const YAML::Node& yaml) noexcept -> std::expected<void, std::string> {
        ...
        rune_detector.config.engine_path.clear();
        if (const auto rune = yaml["rune_network"]) {
            if (rune["engine"]) rune_detector.config.engine_path = rune["engine"].as<std::string>();
            if (rune["score_threshold"])
                rune_detector.config.score_threshold = rune["score_threshold"].as<float>();
            if (rune["keypoint_threshold"])
                rune_detector.config.keypoint_threshold = rune["keypoint_threshold"].as<float>();
            if (rune["center_distance"])
                rune_detector.config.center_distance = rune["center_distance"].as<float>();
        }
        if (rune_detector.config.engine_path.empty()) {
            return std::unexpected {
                "缺少 rune_network.engine 配置（TensorRT engine 路径）" };
        }
        if (!rune_detector.initialize()) {
            return std::unexpected { "无法加载Rune TensorRT engine: " +
                                     rune_detector.config.engine_path };
        }
...
auto Detector::update_camera(const std::array<double, 9>&) noexcept -> void {}
auto Detector::update_camera(const std::array<double, 5>&) noexcept -> void {}
