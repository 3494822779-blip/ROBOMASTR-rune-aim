...
struct VirtualRuneModel::Impl {
    ...
    auto update(Timestamp timestamp) noexcept -> void {
        ...
        // P2-6：观测退化模拟 —— 像素高斯噪声 + 每片符叶独立漏检/遮挡
        std::normal_distribution<double> noise(0.0, config.pixel_noise_px);
        std::uniform_real_distribution<double> unit(0.0, 1.0);
        const auto corrupt = [&](std::optional<Point2d> pixel) -> std::optional<Point2d> {
            if (!pixel) return std::nullopt;
            if (config.pixel_noise_px <= 0.0) return pixel;
            return Point2d { pixel->x + noise(gen_), pixel->y + noise(gen_) };
        };

        // R 标：kPoints[0] = kIcon（y=z=0，旋转恒等，θ=0 即可）
        const auto icon_world = to_world(0.0, RunePagePoints::kPoints[0]);
        if (unit(gen_) >= config.dropout_prob) {
            if (const auto pixel = corrupt(project(icon_world))) {
        ...
        for (const auto blade : inactive_blades) {
            // P2-6：整片漏检/遮挡
            if (unit(gen_) < config.dropout_prob) continue;
            ...
            const auto center_pixel = corrupt(project(blade_center));
            ...
            bullseye.score  = 1.0;
            bullseye.activation = RuneBullseye::Activation::Inactive; // 虚拟符只生成待击打符叶
            ...
            for (const auto& [corner_index, corner_world] : corner_worlds | std::views::enumerate) {
                bullseye.corners[corner_index] =
                    corrupt(project(corner_world)).value_or(bullseye.center);
            }
